...
function unpairPressed()
    ...
    local pressed = gpio.read(UnpairPin) == (UnpairActiveHigh and 1 or 0)
    if not pressed then
        -- Already released
        shortPressUnpair()
    else
        ...
        gpio.trig(UnpairPin, UnpairActiveHigh and gpio.INTR_DOWN or gpio.INTR_UP, unpairReleased)
    end
end
...
function listen()
    ...
    local function gotData(sock, payload)
        ...
        if not rootPemFile then
            rootPemFile = assert(file_open("root.pem", "w"))
        end
...
function writeDecryptedRleToLineFormat(decrypted, filename)
    ...
    local kLookupTable = panel.rleLookupTable
